...
public class SpeechRecognition extends CordovaPlugin {
                                          ...
                                          int[] grantResults) throws JSONException
    {
        for(int r:grantResults)
        {
            ...
            {
                fireErrorEvent("Permition Denied", "Permition Denied.");
                fireEvent("end");
                return;
            }
        }
    ...
    private void fireErrorEvent(String error, String message) {
        JSONObject event = new JSONObject();
        try {
            event.put("type","error");
			event.put("error", error);
			event.put("message", message);
			Log.d("JAVA ERROR ", message);
        } catch (JSONException e) {
            // this will never happen
        }
        ...
        public void onError(int error) {
			...
			switch(error){
				case SpeechRecognizer.ERROR_NETWORK_TIMEOUT:
					fireErrorEvent("network timeout", "Network operation timed out.");					
					break;
				case SpeechRecognizer.ERROR_NETWORK:
					fireErrorEvent("network error", "Unknown Network Error");					
					break;
				case SpeechRecognizer.ERROR_AUDIO:
					fireErrorEvent("audio error", "Audio recording error.");						
					break;
				case SpeechRecognizer.ERROR_SERVER:
					fireErrorEvent("Server Error", "Server sends error status.");						
					break;
				case SpeechRecognizer.ERROR_CLIENT:
					fireErrorEvent("Client Error", "Unknown Client Error.");					
					break;
				case SpeechRecognizer.ERROR_SPEECH_TIMEOUT:
					fireErrorEvent("Speech Timeout", "No speech input");						
					break;
				case SpeechRecognizer.ERROR_NO_MATCH:
					fireErrorEvent("No Match", "No recognition result matched.");						
					break;
				case SpeechRecognizer.ERROR_RECOGNIZER_BUSY:
					fireErrorEvent("Service Busy", "RecognitionService busy.");					
					break;
				case SpeechRecognizer.ERROR_INSUFFICIENT_PERMISSIONS:
					fireErrorEvent("Insufficient Privileges", "Insufficient Privileges");						
					break;
				default:
					fireErrorEvent("Unknown Error", "Unknown Error.");						
					break;
			}
			fireEvent("end");
